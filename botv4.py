        ...
        try:
            async with session.post(DISCORD_WEBHOOK, json={"content": text}, timeout=10) as response:
                ...
                if response.status == 204:
                    ...
                    if discord_fallback_mode:
                        discord_fallback_mode = False
                        recovery_msg = "✅ Discord восстановлен! Возврат к нормальной работе."
                        print(f"[{datetime.utcnow()}] {recovery_msg}")
                        await notify_devs(recovery_msg)
                    success = True
                    return
...
async def handle_admin_command(msg: types.Message):
    global discord_503_count, discord_fallback_mode, discord_last_failure_time
    ...
    if text == "/discord_reset":
        discord_503_count = 0
        discord_fallback_mode = False
        discord_last_failure_time = None
        status_msg = "✅ Discord режим восстановлен\n"
        status_msg += f"503 счетчик: {discord_503_count}\n"
        status_msg += f"Fallback режим: {'Включен' if discord_fallback_mode else 'Выключен'}\n"
        status_msg += f"Последняя ошибка: {'Нет' if discord_last_failure_time is None else 'Сброшена'}"
        await msg.answer(status_msg)
        await notify_devs("🔄 Discord режим восстановлен вручную")
        return

    if text == "/discord_status":
        now = datetime.utcnow()
        status_msg = "📊 Discord статус:\n"
        status_msg += f"503 счетчик: {discord_503_count}/3\n"
        status_msg += f"Fallback режим: {'🔴 Включен' if discord_fallback_mode else '🟢 Выключен'}\n"
        status_msg += f"Fallback чат ID: {FALLBACK_CHAT_ID}\n"
        
        if discord_last_failure_time:
            time_since_failure = now - discord_last_failure_time
            minutes_since = int(time_since_failure.total_seconds() // 60)
            status_msg += f"Последняя ошибка: {minutes_since} мин назад\n"
            
            if discord_fallback_mode:
                time_until_retry = timedelta(hours=1) - time_since_failure
                if time_until_retry.total_seconds() > 0:
                    minutes_until = int(time_until_retry.total_seconds() // 60)
                    status_msg += f"Повторная попытка через: {minutes_until} мин"
                else:
                    status_msg += f"Повторная попытка: при следующем сообщении"
        else:
            status_msg += f"Последняя ошибка: Нет"
    ...
    await msg.answer(
        ...
        "/discord_reset — reset Discord fallback mode\n"
    )
